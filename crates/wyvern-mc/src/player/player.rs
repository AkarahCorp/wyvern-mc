...
use voxidian_protocol::{
    packet::{PacketBuf, PacketEncode, PrefixedPacketEncode, Stage},
    value::VarInt,
};
...
impl Player {
    ...
    pub async fn set_stage(&self, stage: Stage) {
        let _ = self
            .messenger
            .send(ConnectionMessage::SetStage(stage))
            .await;
    }
    ...
    pub async fn write_packet<P: PrefixedPacketEncode + Debug>(&self, packet: P) {
        ...
        let _ = self
            .messenger
            .send(ConnectionMessage::SendPacket(len_buf))
            .await;
        let _ = self
            .messenger
            .send(ConnectionMessage::SendPacket(buf))
            .await;
    }
